def test_create_user(client, test_seed_data, authenticate_demo_user):
	...
	response = client.post("/user", json=new_user, headers=authenticate_demo_user)
...
def test_search_users(client, test_seed_data):
	response = client.get("/user/search/", params={"query": "Marian", "threshold": 60})
	assert response.status_code == 200
	data = response.json()
	assert isinstance(data, list)
	assert any(user["name"] == "Mariana" for user in data)
...
def test_update_user(client, test_seed_data, authenticate_demo_user):
	...
	response = client.put(f"/user/1", json=updated_user, headers=authenticate_demo_user)
	assert response.status_code == 200
	data = response.json()
...
def test_delete_user(client, test_seed_data, authenticate_demo_user):
	response = client.delete(f"/user/1", headers=authenticate_demo_user)
	assert response.status_code == 200
